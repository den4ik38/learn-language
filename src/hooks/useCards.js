...
export function useCards() {
  ...
  const searchRow = ref('')
  const redactedWords = ref([])
  const editedWord = ref([])
  const wordsToExercise = ref([])
  const searchString = () => {
    redactedWords.value = []
    cardsArray.forEach((item)=>{
      if (item.word.includes(searchRow.value)){
        redactedWords.value.push(item) 
      }
    }) 
  }
  const edit= (event) => {
    editedWord.value = []
    for (let i=0; i<redactedWords.value.length; i++) {
      if (redactedWords.value[i].word === event.target.textContent) {
        editedWord.value.push(redactedWords.value[i])
      }
    }
  }
  const addToExercise = (event)=> {
    for (let i=0; i<redactedWords.value.length; i++) {
      if (redactedWords.value[i].word === event.target.textContent) {
        wordsToExercise.value.push(redactedWords.value[i])
      }
    }
  }
  const delForWordsToEx = (id)=> {
    let arr = []
    for (let i=0; i<wordsToExercise.value.length; i++) {
      if (wordsToExercise.value[i].id !== id) {
        arr.push(wordsToExercise.value[i])
      }
    } wordsToExercise.value = arr
  }
  return {
    cards,
    searchRow,
